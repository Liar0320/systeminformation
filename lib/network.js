...
function isEthernetInterfaceConnected(item) {
  const type = (item.type || '').toLowerCase();
  const iface = (item.iface || '').toLowerCase();
  const ifaceName = (item.ifaceName || '').toLowerCase();
  const isWired = type.indexOf('wired') >= 0 || type.indexOf('ethernet') >= 0;
  const isTunnel = iface.startsWith('utun') || ifaceName.startsWith('utun');
  const isLoopback = /^lo\d*$/.test(iface) || /^lo\d*$/.test(ifaceName);

  return isWired && !item.internal && !item.virtual && !isTunnel && !isLoopback && item.operstate === 'up';
}

function ethernetConnected(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      networkInterfaces()
        .then((interfaces) => {
          const connected = interfaces.some(isEthernetInterfaceConnected);
          if (callback) {
            callback(connected);
          }
          resolve(connected);
...
exports.ethernetConnected = ethernetConnected;
    ...
    process.nextTick(() => {
      networkInterfaces().then((interfaces) => {
        ...
        const candidates = interfaces.filter((iface) => {
          ...
          // 排除特殊地址段
          if (iface.ip4.startsWith('127.')) {
            return false;
          }
          if (iface.ip4.startsWith('169.254.')) {
            return false;
          }
          ...
          if (ifaceName.startsWith('tap')) {
            return false;
          }
          if (ifaceName.startsWith('ppp')) {
            return false;
          }
        ...
        if (candidates.length > 0) {
          ...
          candidates.sort((a, b) => {
            ...
            // 2. IP地址段优先级：192.168 > 10 > 172.16-31 > 其他
            ...
            return getPriority(b.ip4) - getPriority(a.ip4);
          });

          const selected = candidates[0];
          result.ip4 = selected.ip4;
          result.ip6 = selected.ip6;
          result.iface = selected.iface;
        }
