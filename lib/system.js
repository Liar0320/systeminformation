    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        const psScript = `
          ...
          $processedApps = @()
          foreach ($app in $apps) {
            # Process path: use InstallLocation or extract from DisplayIcon
            $installPath = $app.InstallLocation
            if ([string]::IsNullOrEmpty($installPath) -and $app.DisplayIcon) {
          ...
          $processedApps | Select-Object -Unique DisplayName, InstallLocation, DisplayVersion, DisplayIcon | ConvertTo-Json
        `;
