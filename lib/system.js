...
// --------------------------
// SYSTEM - scanInstalledApps

function scanInstalledApps(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      const result = [];

      if (_windows) {
        // Windows: Scan registry for installed applications
        const psScript = `
          $apps = @()

          function Resolve-LaunchPath {
            param([Parameter(Mandatory = $false)][string]$DisplayIcon)

            if ([string]::IsNullOrWhiteSpace($DisplayIcon)) {
              return ''
            }

            $iconValue = [Environment]::ExpandEnvironmentVariables($DisplayIcon).Trim().Trim('"')
            $iconValue = ($iconValue -replace ',\\s*-?\\d+$', '')

            if ($iconValue -match '^(?<file>[A-Za-z]:\\\\[^"]+\\.(exe|ico|dll))(\\s+.*)?$') {
          ...
          try {
            ...
            # 当前 DisplayIcon 无法解析时忽略，不影响其他安装项扫描
            return ''
          }
          ...
          $processedApps | Select-Object -Unique DisplayName, InstallLocation, RawInstallLocation, LaunchPath, DisplayVersion, DisplayIcon | ConvertTo-Json
        `;

        util.powerShell(psScript).then((stdout, error) => {
          if (!error && stdout) {
            try {
              ...
              appList.forEach(app => {
                if (app.DisplayName) {
                  result.push({
                    ...
                    path: app.LaunchPath || '',
                    installLocation: app.RawInstallLocation || '',
        ...
        // macOS: Scan Applications folders
        const appDirs = ['/Applications', '/System/Applications'];
        const homeDir = os.homedir();
        if (homeDir) {
          appDirs.push(`${homeDir}/Applications`);
        }

        let scannedCount = 0;
        const totalDirs = appDirs.length;
        ...
        appDirs.forEach(appDir => {
          fs.readdir(appDir, (err, files) => {
            if (!err && files) {
              files.forEach(file => {
                if (file.endsWith('.app')) {
                  const appPath = `${appDir}/${file}`;
                  const plistPath = `${appPath}/Contents/Info.plist`;

                  try {
                    if (fs.existsSync(plistPath)) {
                      const cmd = `/usr/bin/defaults read "${plistPath.replace('.plist', '')}"`;
                      exec(cmd, (error, stdout) => {
                        if (!error && stdout) {
                          const lines = stdout.toString().split('\n');
                          ...
                          for (let i = 0; i < lines.length; i++) {
                            if (lines[i].includes('CFBundleShortVersionString')) {
                              version = lines[i].split('=')[1]?.trim().replace(/[";]/g, '') || '';
                            }
                            if (lines[i].includes('CFBundleIconFile')) {
                              const iconFile = lines[i].split('=')[1]?.trim().replace(/[";]/g, '') || '';
                              if (iconFile) {
                                icon = `${appPath}/Contents/Resources/${iconFile}`;
                      ...
                      // If no Info.plist, just add basic info
                      result.push({
            ...
            scannedCount++;
            if (scannedCount === totalDirs) {
              // Wait a bit for async exec commands to complete
              setTimeout(() => {
                if (callback) {
                  callback(result);
                }
...
exports.scanInstalledApps = scanInstalledApps;

// --------------------------
// SYSTEM - getRegistryInfo

function getRegistryInfo(regnames, callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      // Normalize input to array
      const regnameArray = Array.isArray(regnames) ? regnames : [regnames];
      const result = [];

      if (_windows) {
        // Windows: Query registry for specific regnames
        const regnameList = regnameArray.map(name => `"${name}"`).join(',');

        const psScript = `
          $regnames = @(${regnameList})
          $results = @()
          ...
          foreach ($regname in $regnames) {
            ...
            $regPaths = @(
              ...
              "HKLM:\\SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\$regname",
              "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\$regname"
            )
            ...
            foreach ($regPath in $regPaths) {
              if (Test-Path $regPath) {
                ...
                if ($app) {
                  $appInfo.installLocation = $app.InstallLocation
                  $appInfo.launchPath = Resolve-LaunchPath -DisplayIcon $app.DisplayIcon
                  $appInfo.path = $appInfo.launchPath

                  # Process icon: only return .ico files
                  if ($app.DisplayIcon) {
                    $displayIcon = $app.DisplayIcon -replace '"', ''
                    if ($displayIcon -match '\\.ico$') {
                      $appInfo.icon = $displayIcon
                    }
                  }

                  # Process version
                  $appInfo.version = $app.DisplayVersion
                  break
                }
              }
            }
        ...
        util.powerShell(psScript).then((stdout, error) => {
          if (!error && stdout) {
            try {
              ...
              // If parsing fails, return null values for all regnames
              regnameArray.forEach(regname => {
                result.push({
                  regname: regname,
                  path: null,
                  installLocation: null,
            ...
            // If PowerShell fails, return null values for all regnames
            regnameArray.forEach(regname => {
              result.push({
                regname: regname,
                path: null,
        ...
        // Unsupported platform - return null values
        regnameArray.forEach(regname => {
          result.push({
            regname: regname,
            path: null,
